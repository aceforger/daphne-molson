...
import { FaHeart, FaAward, FaBook, FaLeaf, FaHistory, FaPalette, FaGlobe, FaCrown } from 'react-icons/fa';
    ...
    <section id="about" className="py-20 bg-white">
      <div className="container-custom mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          <div>
            <div className="bg-gradient-to-br from-rose/20 to-white rounded-2xl p-8 shadow-lg border-l-8 border-primary">
              <FaHeart className="text-primary mb-4" size={32} />
              ...
              <div className="space-y-4">
                {authorData.awards.map((award, idx) => (
                  <div key={idx} className="flex items-start gap-3 pb-3 border-b border-gray-100">
                    <RiMedalLine className="text-primary flex-shrink-0 mt-1" size={20} />
                    <div>
          ...
          <div>
            <div className="bg-primary/5 rounded-2xl p-6 mb-6">
              ...
              <div className="bg-rose/20 rounded-lg p-4 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-gradient-to-br from-rose/10 to-white rounded-xl p-4 text-center border border-primary/10">
                ...
                <div className="flex flex-wrap gap-1 justify-center mt-2">
                  {achievements.slice(0, 3).map((achievement, idx) => (
                    <span key={idx} className="text-xs bg-white px-2 py-1 rounded-full text-gray-600">{achievement}</span>
                  ))}
                </div>
              </div>
              <div className="bg-gradient-to-br from-rose/10 to-white rounded-xl p-4 text-center border border-primary/10">
                <FaGlobe className="text-primary mx-auto mb-2" size={24} />
                <h4 className="font-semibold text-primary">Interests</h4>
                <div className="flex flex-wrap gap-1 justify-center mt-2">
                  {interests.slice(0, 3).map((interest, idx) => (
                    <span key={idx} className="text-xs bg-white px-2 py-1 rounded-full text-gray-600">{interest}</span>
                  ))}
                </div>
              </div>
            </div>
